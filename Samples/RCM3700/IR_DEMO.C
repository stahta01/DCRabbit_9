...
/*********************************************************************
	...
	prototyping board or another controller with IR (such
	...
	2. Compile and run this program on an RCM3700 controller
		in debug mode to observe results.
	3. Press switch S1 on the proto-board to transmit a
		packet.
...
#define S1 (BitRdPortI(PFDR, 4))		//switch S1 input port F bit 4
...
main()
{
   ...
   BitWrPortI(PEDR, &PEDRShadow, 1, 5);	//set high to disable rs232 device


	pktFinitBuffers(BUFFER_COUNT, BUFFER_SIZE);
	pktFopen(IR_BAUDRATE, PKT_CHARMODE, ':', modascii_check);
	WrPortI(SFER, &SFERShadow, 0x10); 		//turn on IrDA encoding
...
void pktFinit()
{
	//set transceiver FIR_SEL low for SIR mode
   //set device MD0,MD1 low
   WrPortI(PEDR, &PEDRShadow, PEDRShadow&0xEC);
	WrPortI(PEDDR, &PEDDRShadow, PEDDRShadow|0x13);  	//set to output
}
