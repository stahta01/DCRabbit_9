...
/********************************************************************
   ...
   Note that for the LED to work, output O0 on the controller board
   should be connected to LED1 on the demo board.
...
/*
 * Determines whether the backlight is on or off
 */
#define DISP_BACKLIGHT 1			// 1 for backlight on, 0 for off
...
int ParseToken(HttpState* state, int *bytesRead)
{
	...
	for (i = 0; i < (sizeof(FORMSpec)/sizeof(FORMType)); i++) {
		...
		if (i < (sizeof(FORMSpec)/sizeof(FORMType) - 1) && (tcp_tick((sock_type *)&state->s) == 0)) {
			return -1;
		}
	}
	return 1;
...
int ParsePost(HttpState* state)
{
	...
	while ((bytesRead < (state->content_length - 2)) && (tcp_tick((sock_type *)state->s) != 0)) {
...
main()
{
	...
	// Initialize port D for led 1 (to be used as an unread message flag)
	WrPortI(PDDDR, &PDDDRShadow, 0x01|PDDDRShadow);	//set bit 0 to output
	WrPortI(PDDCR, &PDDCRShadow, 0xfe&PDDCRShadow); //output driven hi/lo
	WrPortI(PDFR, &PDFRShadow, 0xfe&PDFRShadow);		//i/o normal
	WrPortI(PDDR, &PDDRShadow, 0xfe&PDDRShadow);		//unset output 1
	
	keyConfig ( 5, IGNORE,    0, 0, 0,  0, 0);
	keyConfig ( 4, IGNORE,    0, 0, 0,  0, 0);
	keyConfig ( 3, IGNORE,    0, 0, 0,  0, 0);
	keyConfig ( 2, DELETE,    0, 0, 0,  0, 0);
	keyConfig ( 1, EMAIL_UP,  0, 0, 0,  0, 0);
	keyConfig ( 0, SCROLL_UP, 0, 0, 0,  0, 0);

	keyConfig (13, IGNORE,      0, 0, 0,  0, 0);
	keyConfig (12, IGNORE,      0, 0, 0,  0, 0);
	keyConfig (11, IGNORE,      0, 0, 0,  0, 0);
	keyConfig (10, READ,        0, 0, 0,  0, 0);
	keyConfig ( 9, EMAIL_DOWN,  0, 0, 0,  0, 0);
	keyConfig ( 8, SCROLL_DOWN, 0, 0, 0,  0, 0);

	dispBacklight(DISP_BACKLIGHT);
	sock_init();
	http_init();
	tcp_reserveport(80);
