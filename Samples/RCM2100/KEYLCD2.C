...
*/
/***************************************************************************\
	Samples\RCM2100\keylcd2.c

	This program demonstrates a simple setup for a 2x6 keypad and a 2x20 LCD.

	Use with RCM2100 series controllers.  Connect circuitry as described
	in your User Manual Sample Circuit Section.

	When this program starts up, the LCD will display two rows of 6 dots
	...
	as described here and in the example circuitry:
		...
		PD.1	O.C.	Keypad Row 0
		PD.2	O.C.	Keypad Row 1
...
void main (	void	)		
{
	...
	WrPortI ( PDDCR,NULL,0x06 );			//	PD2..PD1 Open Collector
	WrPortI ( PDDDR,NULL,0x06 );			//	PD2..PD1 Output
	...
	for (;;) {
		WrPortI ( PDDR,NULL,0x02 );	//	Scan Row 0
		lcdGoto ( 0,0 );					//	Cursor on column 0, row 0
		keyCol ();							// Scan for keypress
			
		WrPortI ( PDDR,NULL,0x04 );	//	Scan Row 1
