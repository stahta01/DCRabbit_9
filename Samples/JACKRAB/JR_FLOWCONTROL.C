...
*/
/*****************************************
	Samples\Jackrab\jr_flowcontrol.c

	Demonstration of hardware flow control
	Sends a pattern of '*' characters out of port B at 115200 baud.
	Receives one character at a time from port B, and displays it.
	In this example RXC is configured as the CTS input, detecting a clear
	to send condition, and TXC is configured as the RTS output, signaling a
	ready condition. This demonstration can be performed with either one
	or two JackRabbit boards.

	In the case of a single board, simply connect TXB to RXB and connect
	TXC to RXC.
	...
	program normally. Connect Board1:TXB to Board2:RXB, connect ground pins
	together, and connect Board1:RXC to Board2:TXC.
