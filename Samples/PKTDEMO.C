...
/*****************************************************************************
	...
	packet demo, bounces a message back and forth between two JackRabbit boards.
	The RS485 ports of both JackRabbit boards should be tied together..
		RS485-  <--->  RS485-
		RS485+  <--->  RS485+
		GND     <--->  GND

	Similar results can be obtained on boards without RS485 tranceivers
	by wiring port D outputs together.
		PC0  <--->  PC1
		PC1  <--->  PC0

	Hit a button on the dev board to send an initial packet

	receives packets on port D using ':' as a start character,

	format is ':', then 2 bytes for length in hex, followed by data
	e.g.
		:13Thisisapacketofdata

	Uncomment different versions of the pktDopen call below to try out
	different packet schemes. Just make sure that both boards are running
	in the same mode.
...
{
	auto char *bytes;
	auto char ** tailptr;	//not really used, bu trequired for strtol().
	auto char length_field[3];
	auto long length_read;
