...
#class auto
#if _USER
#error "This sample is not compatible with RabbitSys."
#endif
/*
...
int	zmTCPMenu(int Option)
{
   ...
   switch (Option)
   {
      ...
      case 4:	// Option 4 of the Tcp Menu
         ...
         ifconfig(IF_ETH0, IFG_IPADDR, &ipval, IFS_END);
      ...
      case 5:	// Option 5 of the Tcp Menu
         Zmenu_State(0); 		// Turn off the menu system, to have manual
   		...
   		// clear the screen
   		rn_dispClear(zm_device,0);
         ifconfig(IF_ETH0, IFG_ROUTER_DEFAULT, &ipval, IFS_END);
         // move cursor and display heading
      	rn_dispGoto(zm_device,(ZMENU_COLUMNS/2) - \
      						(strlen("-----ROUTER IP-----")/2),
    					0,0);
    	...
    	case 6:	// Option 6 of the Tcp Menu
         Zmenu_State(0); 		// Turn off the menu system, to have manual
   		...
   		// clear the screen
   		rn_dispClear(zm_device,0);
         ifconfig(IF_ETH0, IFG_NETMASK, &ipval, IFS_END);
         // move cursor and display heading
      	rn_dispGoto(zm_device,(ZMENU_COLUMNS/2) - \
      						(strlen("-----NETMASK IS----")/2),
    					0,0);
...
int	zmDataMenu(int Option)
{
   switch (Option)
   {
      ...
      case 3:	// Option 3 of the Data Menu
         ...
         tVal.tm_mday, tVal.tm_mon, tVal.tm_year + 1900,
...
main ()
{
   ...
   ifconfig(IF_ETH0, IFG_IPADDR, &ipVal,
    	...
    	case 0:
         ...
         else if(zm_option.param & ZMENU_DATETIME)
         {
            if (zm_option.param & ZMENU_REFRESH_TIME)
         	...
         	sprintf(zm_DataString,"%02d-%02d-%04d %02d:%02d:%02d",
         	TimeDate.tm_mday, TimeDate.tm_mon, TimeDate.tm_year + 1900,
