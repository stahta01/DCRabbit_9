...
/*******************************************************************************
   ...
   An optional LCD/Keypad Module is required to run this sample program.  The
   brdInit() function in rcm39xx.lib automatically enables the external I/O bus
   for LCD/Keypad operations.

   Description
   ===========
   This sample program implements a web server that allows email messages to be
   entered that are then shown on the LCD display.  The keypad allows the user
   to scroll within messages, flip to other emails, mark messages as read, and
   delete emails.  When a new email arrives an LED (on the LCD/Keypad Module)
   turns on and turns back off once the message has been marked as read.  A log
   of all email actions is kept, which can be displayed in the web browser.  All
   current emails can also be read with the web browser.

   This sample program is adapted from ...\Samples\ICOM\mboxdemo.c.

   Instructions
   ============
   1. If necessary, make changes in the configuration section below to match
      your requirements.

   2. Compile and run this program.

   3. Using your PC's web browser, access the web page running on the
      controller.  Through the web page you can read, send or view messages.

   4. The controller's LCD/Keypad Module's display will indicate if you have
      email and you can do the following by using the keypad to:
...
/*
 *  REDIRECTTO is used to tell the browser which page to
 *  hit next.  The default REDIRECTTO assumes that you are
 *  serving a page that does not have any address
...
/*
 *  Instead of sending other text back from the cgi
...
char *myurl(void)
{
   ...
   sprintf(URL, "http://%s/index.html", inet_ntoa(tmpstr, ipval));
   return URL;
}
