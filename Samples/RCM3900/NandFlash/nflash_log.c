...
/*******************************************************************************
   ...
   hits in the nand flash.  This log can be viewed and cleared from a web
   browser.
   ...
   2. Compile and run this sample program.  If running in debug mode then nand
...
#ximport "nflash_log.c"      flashlog_c

// global variables
int count, date_lock, day, hour, minute, month, second, year;
long log_end_block, log_read_block, log_start_block;
...
/*
** 1) Attempt to intialize the on-board nand flash device.
...
int init_log(void)
{
   ...
   printf("Attempting to initialize the on-board nand flash device.\n");
   if (err = nf_initDevice(&nandFlash, 0)) {
      printf("ERROR %d:  On-board nand flash device init failed!\n\n", err);
      return -1;  // failed to initialize the on-board nand flash device
   }

   printf("\nOn-board nand flash device initialized.\n");
   ...
   for (page = 0L; page < nf_getPageCount(&nandFlash); ++page) {
      ...
      if (err) {
         ...
         xmem2root(&header, myMainBuffer, sizeof(header));
   ...
   if (found_start) {
      ...
      header.datalen = 0L;
      root2xmem(myMainBuffer, &header, sizeof(header));
...
*/
nodebug
int read_log(char *buffer, unsigned bufsize)
{
...
int append_log(char *buffer)
{
   ...
   while (string_index < string_size) {
      ...
      root2xmem(myMainBuffer + (long) sizeof(header) + header.datalen,
                buffer + string_index, writesize);
      header.datalen += writesize;
      root2xmem(myMainBuffer, &header, sizeof(header)); //update header
