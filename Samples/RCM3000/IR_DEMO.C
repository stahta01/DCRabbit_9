...
/**********************************************************
	...
	RCM3000 prototyping boards to be set up so that their IR
	transceivers are facing each other.
	
	This program should be run in standalone mode on one
	RCM3000 and then in debug mode on the other to observe
	the results.
...
unsigned char parse_hex(char *ascii)
{
	...
	/* Since "byte_value" shifted twice, we don't need to init it to zero! */
...
void pktFinit()
{
	...
	WrPortI(PGDR, &PGDRShadow, PGDRShadow & 0xcb); //set MD0, MD1, and TXD low
	WrPortI(PGDDR, &PGDDRShadow, PGDDRShadow | 0x34); //set to outputs
}
