...
// 	RETURN: 	TRUE if keypress, else FALSE if none without "delay" millis.
//------------------------------------------------------------------------
nodebug
...
void DemoBd_Demo(void)
{
	...
	costate
	{
		...
		if (Switches_Active)
		{
			...
			{
				case 1:
					WrPortI(PGDR, &PGDRShadow, PGDRShadow^DS1);  //toggle DS1
					break;

				case 2:
					WrPortI(PGDR, &PGDRShadow, PGDRShadow^DS2);  //toggle DS2
					break;
			}
		}
	}
