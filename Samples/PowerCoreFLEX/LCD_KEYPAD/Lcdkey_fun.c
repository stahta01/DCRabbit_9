...
/**************************************************************************
   ...
   prototyping board.
	...
	(Adapted from \Samples\LCD_Keypad\122x32_1x7\fun.c)

  	Instructions
  	============
  	1. Run and compile this program.
  	2. Watch the LCD display as it goes through the various graphic demo's.
  	3. At any given time press a key on the LCD/keypad module to see the
  	   corresponding LED light-up.
	4. Press S2 to light up protoboard DS5 LED0, or press S3 to light up
   	protoboard DS6 LED1.
...
#define S1  (digIn(0))	//switch, port G bit 4
#define S2  (digIn(1))	//switch, port F bit 1
...
{
	// works for port D bits 6 and 7 only
	BitWrPortI(PDDR, &PDDRShadow, onoff, channel);
}
...
void monitor_switches(void)
{
   ...
   costate
	{
		if(!S1 && S2)								//wait for switch S1 press
		{
			waitfor(DelayMs(50));
			if (!S1)								//wait for switch release
			{
   ...
   costate
	{
		if(!S2 && S1)								//wait for switch S2 press
      {
      	waitfor(DelayMs(50));
			if (!S2)								//wait for switch release
			{
				Switches_Active = TRUE;
				DemoBd_switch = 2;
			}
