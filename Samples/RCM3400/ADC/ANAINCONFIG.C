...
/***************************************************************************
   anainconfig.c
	...
	=============================
	Connect PE7 on J1 connector to CNVRT terminal on J3 connector.
...
//---------------------------------------------------------
// asserts PE7 high as the convert pin for 2 ads7870 cclk's
//---------------------------------------------------------
void adconvert(void)
{
#asm
	ld		a,(PEDRShadow)
	and	07fh				;ensure 2 adc cclks low before asserting
	ioi	ld (PEDR),a
...
main ()
{
	...
	/// set port E bit 7 as the convert pin
	BitWrPortI(PEDR, &PEDRShadow, 0, 7);			//set output low
	BitWrPortI(PEDDR, &PEDDRShadow, 1, 7);			//configure bit 7 to output
